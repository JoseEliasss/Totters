...
const RestaurantList = ({ count, typeFilter }) => {
  const filteredData = typeFilter ? data.filter((r) => r.type === typeFilter) : data; // Show all restaurants if no typeFilter is provided
  ...
  return (
    <div>
      <div className="restaurants-list">
        {displayedData.map((r) => (
          <RestaurantCard key={r.id} favorite={r.favorite} {...r} />
        ))}
      </div>
    </div>
  );
